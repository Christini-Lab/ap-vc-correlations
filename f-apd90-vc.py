...
def plot_figure(feature, heat_currents):
    ...
    sub = grid[3:, :]
    vc_subgrid = sub.subgridspec(2, 2, hspace=.3, wspace=.25, height_ratios=(1, 2))
    ...
    corr_name = 'I6mV'
    plot_vc(v_axs[0], i_axs[0], corr_name, xrange=[-200, 400])
    i_axs[0].set_ylim(-5, 15)
    i_axs[0].set_title(r'$I_{6mV}$', y=.9)

    #corr_name = 'IK1'
    #plot_vc(v_axs[1], i_axs[1], corr_name, xrange=[-100, 25])
    #i_axs[1].set_ylim(-15, 5)
    #i_axs[1].set_title(r'$I_{K1}$', y=.9)

    #corr_name = 'If'
    #plot_vc(v_axs[2], i_axs[2], corr_name, xrange=[-800, 50])
    #i_axs[2].set_ylim(-18, 2)
    #i_axs[2].set_title(r'$I_{f}$', y=.9)

    corr_name = 'IKs'
    plot_vc(v_axs[1], i_axs[1], corr_name, xrange=[-300, 40])
    i_axs[1].set_ylim(-2, 15)
    i_axs[1].set_title(r'$I_{Ks}$', y=.9)
    ...
    for i, ax in enumerate([corr_axs[0], all_ap_ax, v_axs[0], v_axs[1]]):
        ax.set_title(letters[i], y=.98, x=-.1)

    matplotlib.rcParams['pdf.fonttype'] = 42
    plt.savefig('./figure-pdfs/f-ap90-vc.pdf', transparent=True)
...
def plot_cc(ax, cc_type, col=None):
    ...
    ax.plot(t_sh, ap_sh, '#377eb8', rasterized=True, label='Cell 3')
    #ax.plot(t_med, ap_med, '#ff7f00', rasterized=True)
    ax.plot(t_long, ap_long, '#4daf4a', rasterized=True, label='Cell 4')
...
def plot_vc(v_ax, i_ax, corr_name, xrange):
    ...
    for f in all_files:
        ...
        if np.isnan(all_ap_features[all_ap_features['File'] == f].APD90.values[0]):
            continue
    ...
    i_ax.plot(times, vc_long, color='#4daf4a')
...
def main():
    plot_figure('APD90', ['I6mV', 'IK1', 'If', 'IKs'])
